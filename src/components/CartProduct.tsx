...
function CartProduct({ product }: { product: { id: number; qty:number}}) {
  ...
  const changeQtyHandler = (e:React.MouseEvent<HTMLElement, MouseEvent>):void => {
  ...
  >
    ...
    <div className="ms-3 d-flex justify-content-center align-items-ccenter flex-column">
      <h3 className="fs-3">{productData?.name}</h3>
        <small className="text-muted">{currencyFormater(productData.price)}</small>
    </div>
    <div className="d-flex flex-column justify-content-center align-items-center p-2">
      <div className="d-flex" style={{height:"3rem"}}onClick={changeQtyHandler}>
        <Button ref={incermentButton as React.MutableRefObject<HTMLButtonElement>}  style={{width:"2.5rem",height:"2.5rem"}}>+</Button><div className="fs-2 mx-2">{product.qty}</div><Button ref={decrementButton as  React.MutableRefObject<HTMLButtonElement>} style={{width:"2.5rem",height:"2.5rem"}}>-</Button>
      </div>
        <p className="mt-2">total price: <span className="text-muted">{currencyFormater((product.qty*productData.price))}</span></p>
    </div>
  </div>
  )
}
